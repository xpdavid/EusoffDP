...
function personal_detail_checker() {
	if ($("#name").val() == "" || $("#phone_num").val() == "" || $("#email").val() == "") {
...
function compute_total_price() {
	...
	$("#item_price").html(total_item.toFixed(2));
	var total_ticket = compute_total();
	$("#total_price").html((total_ticket + total_item).toFixed(2));
	return (total_ticket + total_item).toFixed(2)
}
...
function checkout() {
	...
	pre_data["items"] = items_data;
	pre_data["additional_info"] = {
		flower_to_performers : $("#performers_name").val()
	};
...
// item object
function item(name, unit_price, id) {
	this.name = name;
...
item.prototype.update_display = function() {
